...
class BeanstreamGateway(Gateway):
    test_url = "https://www.beanstream.com/scripts/process_transaction.asp"
    live_url = "https://www.beanstream.com/scripts/process_transaction.asp"
    ...
    homepage_url = "http://www.beanstream.com/"
    display_name = "Beanstream"

    def __init__(self):
        self.merchant_id = settings.BEANSTREAM_MERCHANT_ID
    ...
    def add_creditcard(self, post, credit_card):
        ...
        post["trnExpMonth"] 	= credit_card.expire_date.expiration().strftime("%m")
        post["trnExpYear"] 	= credit_card.expire_date.expiration().strftime("%y")
        post['trnCardNumber']   = credit_card.number
        post['trnCardCvd']  	= credit_card.verification_value
        post['trnCardOwner']  = "%s %s" % (credit_card.first_name, credit_card.last_name)
    ...
    def authorize(self, money, credit_card, options = None):
        """Using Beanstream payment gateway, authorize the
        credit card for specified money"""
        if not options:
            options = {}
        if not self.validate_card(credit_card):
